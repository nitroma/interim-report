...
\paragraph{Decanter}
 The size of the decanter was based on the time required for separation. 
\begin{equation}
    t = \left[\frac{100\mu}{(\rou_A - \rou_B)}\right]
    \label{decanter_time_sizing}
\end{equation}
where t is separation time, $\mu$ is viscosity of the continuous phase, $\rou_A$ and $\rou_B$ are the densities of the liquid A and B to be separated. 
\begin{equation}
    Total liquid hold up = (V_A + V_B) * t
    \label{decanter_holdup_sizing}
\end{equation}
where $V_A$ and $V_B$ are the volumetric flowrate of the liquids. The vessel should be 95 % 
